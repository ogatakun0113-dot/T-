...
if type_choice == "T型":
    ...
    st.subheader(f"🖼️ {type_choice} の抵抗配置イメージ")
    st.code(f"""
...
c1, c2 = st.columns(2)
if type_choice == "T型":
    c1.metric("直列抵抗 R1 (左右)", f"{r_series:.2f} Ω")
    c2.metric("並列抵抗 R2 (中央)", f"{r_shunt:.2f} Ω")
else:
    c1.metric("並列抵抗 R1 (両端)", f"{r_shunt:.2f} Ω")
    c2.metric("直列抵抗 R2 (中央)", f"{r_series:.2f} Ω")
st.markdown('</div>', unsafe_allow_html=True)

# --- 3. 周波数特性のシミュレーション（標準機能のみ） ---
st.markdown("---")
st.subheader("📉 周波数特性の目安 (イメージ)")

# 外部ツールを使わず、Streamlit標準のグラフ機能を使用
chart_data = []
for i in range(1, 11):
    freq = i * 100 # 100MHz刻みで1GHzまで
    # 70MHz帯はほぼ理想通り、それ以上はリード線の影響でロスが増える想定
...
st.info("**緒方さんへの現場メモ：** 70MHz付近なら理論通りですが、それ以上の高周波ではハンダの「山」や「抵抗の足」の長さだけで数dB変わります。可能な限り短く実装するのが成功の鍵です！")
